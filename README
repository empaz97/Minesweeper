...
There is also a backup .jar file with an older version of the project. This backup can be found at:
    ...
    NOTE: I don't want this version graded since it is missing a handful of things, but I am including it as a backup in case my newest version breaks during grading
    
The image resources were all created by me and are located in the resources/images/ folder within the game directory
The sound files that I use in the project are from freesoundeffects.com and are located in the resources/sounds/ folder within the game directory, and a link to their licence agreement can be found here:
    https://www.freesoundeffects.com/licence.php
The high score data is saved and loaded via serialization. The serialized (.ser) files are located in the highScores/ folder within the game directory. These can be reset in-game

Under the Game menu, you can choose to start a new game, or set the difficulty which starts a new game with that difficulty. The difficulties are as follows:
...
What I couldn't get to work:
    - Expert controls (chording, keyboard controls, etc) have not been included
